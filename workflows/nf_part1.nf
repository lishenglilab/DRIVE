...
/*
========================================================================================
    SUBWORKFLOW MODULE (Part 1 Models) - V8.1 Final Fix
...
process setup_part1_env {
    ...
    // 【重要】: 这里的 local_wheels_dir 必须从工作流中传入
    input:
    ...
    echo "Part 1 Environment is fully prepared." > env_ready.txt
    """
}
...
workflow run_part1_workflow {
    ...
    main:
        ...
        setup_part1_env(local_wheels_dir)
        def env_ready_signal = setup_part1_env.out
        ...
        // 所有计算进程现在都不再需要 local_wheels_dir
        run_GPDRP(
            gpu_id, output_dir,
            gpdrp_models_ch.map { it[0] },
            gpdrp_models_ch.map { it[1] },
            input_params, env_ready_signal
        )

        run_BANDRP(gpu_id, output_dir, input_params, env_ready_signal)
        run_DeepTTC(gpu_id, output_dir, input_params, env_ready_signal)
        run_paccmann(gpu_id, output_dir, input_params, env_ready_signal)
        run_Precily(gpu_id, output_dir, input_params, env_ready_signal)
}
