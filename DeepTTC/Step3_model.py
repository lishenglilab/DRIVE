...
import os
import numpy as np
from sklearn.metrics import mean_squared_error, r2_score  # <-- 1. 导入 r2_score
...
class DeepTTC:
    def __init__(self, modeldir):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # <-- 修复设备不匹配问题
    ...
    def train(self, train_drug, train_rna, val_drug, val_rna):
        ...
        valid_metric_record = []
        # <-- 3.1. 更新表头
        valid_metric_header = ['# epoch', "MSE", 'RMSE', "R-squared",
        ...
        for epo in range(train_epoch):
            ...
            with torch.set_grad_enabled(False):
                ...
                # <-- 3.3. 更新列表
                lst = ["epoch " + str(epo)] + list(map(float2str, [mse, rmse, r2, person, p_val, spearman,
                                                                   s_p_val, CI]))
                valid_metric_record.append(lst)

                # <-- 3.4. 更新打印和 TensorBoard
                if mse < max_MSE:
                    model_max = copy.deepcopy(self.model)
                    max_MSE = mse
                    ...
                    writer.add_scalar("valid/r2_score", r2, epo)
    ...
    def predict(self, drug_data, rna_data):
        ...
        return y_label, y_pred, mse, rmse, r2, person, p_val, spearman, s_p_val, CI
...
if __name__ == '__main__':
    ...
    # traindata, testdata = obj.Getdata.ByBinary(random_num=1)
    traindata, testdata = obj.Getdata.ByDrug(random_seed=666)
    # traindata, testdata = obj.Getdata.ByRandom(random_seed=2)
    # encoding 完成
