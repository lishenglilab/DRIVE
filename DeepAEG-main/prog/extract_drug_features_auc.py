...
def _drug2emb_encoder(smile):
    ...
    vocab_path = "../prog/ESPF/drug_codes_chembl_freq_1500.txt"
    sub_csv = pd.read_csv("../prog/ESPF/subword_units_map_chembl_freq_1500.csv")
...
if __name__  == '__main__':
    ...
    if (args.use_aug):
        ...
        count_drug=0
        save_dir = '../mydata/GDSC/50 and 11'
        pubchemid2smile = {item.split('\t')[0]: item.split('\t')[1].strip() for item in
                           open(drug_smiles_file).readlines()}
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
