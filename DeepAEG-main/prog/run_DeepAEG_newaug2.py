...
DPATH = '../mydata'
Drug_info_file = '%s/GDSC/durg_m.csv' % DPATH
...
Gene_info_file='%s/CCLE/Result_StandardScaler2.csv' % DPATH
...
def DataSplit_drug(data_idx, ratio=0.8):
    ...
    for sample in data_idx:
        cellline, pubchem_id, ln_IC50, cancer_type = sample
        drug_to_samples[pubchem_id].append(sample)

    # 按比例划分药物组
    drugs = list(drug_to_samples.keys())
    random.shuffle(drugs)  # 随机打乱药物顺序
    num_drugs = len(drugs)
    num_train_drugs = int(ratio * num_drugs)

    train_drugs = set(drugs[:num_train_drugs])
    test_drugs = set(drugs[num_train_drugs:])
...
def DataSplit_cell(data_idx, ratio=0.8):
    ...
    random.shuffle(celllines)  # 随机打乱细胞系顺序
    num_celllines = len(celllines)
    num_train_celllines = int(ratio * num_celllines)

    train_celllines = set(celllines[:num_train_celllines])
    test_celllines = set(celllines[num_train_celllines:])
    ...
    for cellline, samples in cellline_to_samples.items():
        if cellline in train_celllines:
            data_train_idx.extend(samples)
        elif cellline in test_celllines:
            data_test_idx.extend(samples)
...
def ModelEvaluate(model, X_drug_data_test,  X_cell_line_test, Y_test):
    ...
    print("The overall Spearman's correlation is %.4f." % spearmanr_pcc)
    print("The overall RMSE is %.4f." % rmse)
    print("The overall R^2 is %.4f." % r2)


    # 保存结果到CSV文件
    file_path = "evaluation_results.csv"
    header = ["Pearson Correlation", "Spearman Correlation", "RMSE", "R^2"]
    row = [overall_pcc, spearmanr_pcc, rmse, r2]
...
def main():
    ...
    data_train_idx, data_test_idx = DataSplit_mix(data_idx)
