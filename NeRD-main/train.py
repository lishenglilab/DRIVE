...
def train(modeling, train_batch, val_batch, test_batch, lr, epoch_num, cuda_name, i):
    ...
    val_pearsons = []
    val_r2 = []
    train_data = TestbedDataset(root='mydata', dataset='train_blind_drug{num}'.format(num=i))
    val_data = TestbedDataset(root='mydata', dataset='val_blind_drug{num}'.format(num=i))
    test_data = TestbedDataset(root='mydata', dataset='test_blind_drug{num}'.format(num=i))
    ...
    for epoch in range(epoch_num):
        ...
        val_r2.append(ret[4])
        # 确保目录存在，如果没有则创建
        result_dir = 'result{num}'.format(num=i)
        if not os.path.exists(result_dir):
            os.makedirs(result_dir)
        if ret[1] < best_mse:
            torch.save(model.state_dict(), os.path.join(result_dir, 'model.model'))
            with open(os.path.join(result_dir, 'result.csv'), 'w') as f:
                f.write(','.join(map(str, ret_test)))
            with open(os.path.join(result_dir, 'metrics.txt'), 'w') as f:
                f.write(f'Best Epoch: {epoch + 1}\n')
                f.write(f'Best MSE: {ret[1]}\n')
                f.write(f'R2: {ret[4]}\n')  # R2的值
                f.write(f'RMSE: {ret[0]}\n')  # RMSE的值
                f.write(f'Spearman: {ret[3]}\n')  # Spearman相关系数的值
                f.write(f'Best Pearson: {ret[2]}\n')
            best_epoch = epoch + 1
            best_mse = ret[1]
            best_pearson = ret[2]
