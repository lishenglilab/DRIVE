...
parser = argparse.ArgumentParser(
    description="Predict drug response (IC50) for new drugs and new cell lines using a pre-trained DeepCDR model.",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
parser.add_argument("--model_file", type=str, required=True, help="Path to the pre-trained Keras model file (.h5)")
parser.add_argument("--drugs_file", type=str, required=True,
                    help="Path to the CSV file containing new drug information (name, smiles)")
parser.add_argument("--output_file", type=str, required=True, help="Path to the CSV file to save prediction results")
parser.add_argument("--mut_file", type=str, required=True, help="Path to the mutation data CSV file for new cell lines")
parser.add_argument("--gexp_file", type=str, required=True,
                    help="Path to the gene expression data CSV file for new cell lines")
parser.add_argument("--methy_file", type=str, required=True,
                    help="Path to the methylation data CSV file for new cell lines")
parser.add_argument("--align_gexp_file", type=str, required=True,
                    help="Path to the gene expression file from the original training set (for alignment)")
parser.add_argument("--align_mut_file", type=str, required=True,
                    help="Path to the mutation file from the original training set (for alignment)")
...
try:
    ...
    print("Error: mygene library is not installed. Please run 'pip install mygene' to install it.")
    exit(1)

import keras
import keras.backend as K
from keras.models import Model, load_model
from keras.layers import (Input, Dense, Activation, Dropout, Flatten, Concatenate, BatchNormalization, Lambda,
...
# =============================================================================
# 2. Drug Feature Generation Module
# =============================================================================
...
def local_get_atom_implicit_valence_one_hot(atom: RDKitAtom,
                                            allowable_set: List[int] = DEFAULT_ATOM_IMPLICIT_VALENCE_SET,
                                            include_unknown_set: bool = True) -> List[float]: return one_hot_encode(
    atom.GetImplicitValence(), allowable_set, include_unknown_set)


def local_get_atom_explicit_valence_one_hot(atom: RDKitAtom,
...
def _construct_atom_feature(atom: RDKitAtom, h_bond_infos: List[Tuple[int, str]], use_chirality: bool,
    ...
    hybridization = get_atom_hybridization_one_hot(atom, allowable_set=_USER_HYBRIDIZATION_SET,
    ...
    total_num_Hs = get_atom_total_num_Hs_one_hot(atom, allowable_set=_DEFAULT_TOTAL_NUM_Hs_SET,
                                                 include_unknown_set=True)
    atom_feat = np.concatenate(
        [atom_type, formal_charge, hybridization, acceptor_donor, aromatic, degree, total_num_Hs])
    if True:
        imp_valence = local_get_atom_implicit_valence_one_hot(atom, DEFAULT_ATOM_IMPLICIT_VALENCE_SET,
                                                              include_unknown_set=True)
        exp_valence = local_get_atom_explicit_valence_one_hot(atom, DEFAULT_ATOM_EXPLICIT_VALENCE_SET,
                                                              include_unknown_set=True)
        atom_feat = np.concatenate([atom_feat, imp_valence, exp_valence, [float(atom.HasProp('_ChiralityPossible')),
...
class MolGraphConvFeaturizer(MolecularFeaturizer):
    ...
    def _get_atom_feature_dim(self):
        ...
        try:
            ...
            raise RuntimeError(f"Could not dynamically determine atom feature dimension: {e}")
...
class GraphLayer(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        features, edges_in = inputs;
        edges_float = K.cast(edges_in, K.floatx())
...
class GraphConv(GraphLayer):
    ...
    def get_config(self):
        config = {'units': self.units, 'kernel_initializer': keras.initializers.serialize(self.kernel_initializer),
                  ...
                  'kernel_constraint': keras.constraints.serialize(self.kernel_constraint), 'use_bias': self.use_bias,
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(shape=(feature_dim_in, self.units), initializer=self.kernel_initializer,
                                 regularizer=self.kernel_regularizer, constraint=self.kernel_constraint,
                                 name=f'{self.name}_W')
        if self.use_bias: self.b = self.add_weight(shape=(self.units,), initializer=self.bias_initializer,
...
def CalculateGraphFeat(node_features_raw, edge_index_raw, max_atoms_val, model_expected_atom_feat_dim):
    ...
    if feat_dim > model_expected_atom_feat_dim:
        node_features = node_features_raw[:, :model_expected_atom_feat_dim]
        ...
        node_features = node_features_raw
    adj = np.zeros((num_nodes, num_nodes));
    if num_nodes > 0 and edge_index_raw.shape[1] > 0: adj[edge_index_raw[0], edge_index_raw[1]] = 1
    padded_adj = np.zeros((max_atoms_val, max_atoms_val));
    padded_feat = np.zeros((max_atoms_val, model_expected_atom_feat_dim))
    if num_nodes > 0:
...
def build_gene_map_with_mygene(gene_symbols: list):
    print(f"Querying mygene.info for Entrez IDs of {len(gene_symbols)} gene symbols...");
    mg = mygene.MyGeneInfo()
    results = mg.querymany(gene_symbols, scopes='symbol', species='human', fields='entrezgene', as_dataframe=True,
                           verbose=False)
    results.dropna(subset=['entrezgene'], inplace=True);
    results = results[~results.index.duplicated(keep='first')]
    gene_map = results['entrezgene'].astype(int).astype(str).to_dict()
    print(f"Query complete. Successfully mapped {len(gene_map)} gene symbols to Entrez IDs.");
    return gene_map


def load_and_align_new_cell_data(new_omics_files: Dict[str, str], train_gene_id_lists: Dict[str, List[str]]):
    print("Loading and aligning new cell line omics data...");
    all_gene_symbols, raw_dfs = set(), {}
    for omic, path in new_omics_files.items():
        if path and os.path.exists(path):
            try:
                df = pd.read_csv(path, index_col=0); df.index = df.index.str.strip(); all_gene_symbols.update(
                    df.columns); raw_dfs[omic] = df
            except Exception as e:
                print(f"Failed to read file {path}: {e}")
        elif path:
            print(f"Warning: File {path} not found")
    if not all_gene_symbols: print("Error: Failed to read any gene names"); return {}
    gene_map = build_gene_map_with_mygene(list(all_gene_symbols))
    if not gene_map: print("Error: Gene name mapping failed"); return {}
    aligned_data = {}
    for omic, df in raw_dfs.items():
        df.rename(columns=gene_map, inplace=True)
        train_ids = train_gene_id_lists.get(omic)
        if not train_ids: print(f"Warning: Could not find the training gene list for {omic}"); continue
        aligned_df = df.reindex(columns=train_ids, fill_value=0);
        aligned_df.index = aligned_df.index.astype(str)
        print(f"Aligned {omic} data to {aligned_df.shape[1]} training gene IDs");
        aligned_data[omic] = aligned_df
    return aligned_data


# =============================================================================
# 5. Unified Prediction Main Workflow
# =============================================================================
def predict_for_new_drugs_and_cell_lines(
    ...
    print("=" * 50 + "\nStarting unified prediction workflow (new drugs + new cell lines)...\n" + "=" * 50)
    ...
    try:
        ...
        print(
            f"Based on the pre-trained model '{os.path.basename(model_file_path)}', setting drug atom feature dimension to: {MODEL_EXPECTED_DRUG_DIM}")
    except Exception as e:
        print(f"Error: Failed to load model: {e}"); return

    print("\n--- Step 1: Processing New Drugs ---")
    try:
        new_drugs_df = pd.read_csv(new_drugs_file_path, header=None, names=['drug_name', 'smiles']); print(
            f"Read {len(new_drugs_df)} new drug records from {new_drugs_file_path}.")
    except FileNotFoundError:
        print(f"Error: New drug file {new_drugs_file_path} not found."); return

    print("\n--- Step 2: Processing New Cell Lines ---")
    try:
        train_mut_df_cols = pd.read_csv(data_paths_for_align['train_mut'], index_col=0, nrows=0).columns.astype(
            str).tolist()
        train_gexp_df_cols = pd.read_csv(data_paths_for_align['train_gexp'], index_col=0, nrows=0).columns.astype(
            str).tolist()
        train_gene_id_lists = {'mut': train_mut_df_cols, 'gexp': train_gexp_df_cols, 'methy': train_mut_df_cols}
    except Exception as e:
        print(f"Error: Failed to load gene lists for alignment: {e}"); return
    ...
    if not active_omic_types: print("Error: No omics data files provided"); return

    print("\n--- Step 2b: Calculating Intersection of Cell Line IDs ---")
    cell_line_id_sets = {omic: set(df.index) for omic, df in aligned_cell_data.items() if
                         omic in active_omic_types and not df.empty}
    if not cell_line_id_sets: print("Error: Could not load cell line data from any of the provided files"); return
    for omic, ids in cell_line_id_sets.items(): print(f" - Found {len(ids)} cell line IDs in '{omic}' data.")

    intersected_ids = set.intersection(*cell_line_id_sets.values())
    new_cell_line_ids = sorted(list(intersected_ids))
    if not new_cell_line_ids: print("\nError: No common cell line IDs among all provided omics data files"); return

    print(f"\nSuccess! Predictions will be made for {len(new_cell_line_ids)} common cell lines.")
    print("\n--- Step 3: Starting Combined Prediction ---")
    predictions = []
    with tqdm(total=len(new_drugs_df) * len(new_cell_line_ids), desc="Overall Prediction Progress") as pbar:
        for _, drug_row in new_drugs_df.iterrows():
            ...
            try:
                ...
                feat, adj = CalculateGraphFeat(graph_data.node_features, graph_data.edge_index, Max_atoms,
                                               MODEL_EXPECTED_DRUG_DIM)
                drug_input = [np.array([feat]), np.array([adj])]
            except Exception as e:
                print(f"Warning: Featurization failed for drug {drug_name}: {e}"); pbar.update(
                    len(new_cell_line_ids)); continue

            for cell_id in new_cell_line_ids:
                model_inputs = list(drug_input)
                try:
                    # Dynamically build the input list to match the model
                    ...
                    predictions.append({'drug_name': drug_name, 'smiles': smiles, 'cell_line_id': cell_id,
                                        'predicted_ln_IC50': float(pred_val)})
                except Exception as e:
                    print(f"\nError (Drug {drug_name}, Cell line {cell_id}): {e}")
                pbar.update(1)
...
# =============================================================================
def main():
    """Main function to orchestrate the entire prediction workflow"""
    ...
    print(
        f"Drug featurizer initialized, will generate {graph_featurizer._get_atom_feature_dim()}-dimensional atom features.")
