...
# --- Cell & Drug Preprocessing Functions (无变化) ---
def preprocess_new_cell_data(gene_expr_path, canonical_gene_path, bionic_dict_path, gene_add_num, expected_gene_dim):
    ...
    try:
        ...
        print(f"错误: 读取标准基因列表 '{canonical_gene_path}' 失败。正在尝试备用方案...")
...
def main(args):
    ...
    print(f"\n--- Total {len(cell_lines_to_predict)} cell lines loaded for prediction. ---")

    print("\n--- Starting Chunked Prediction ---")
    try:
        chunk_iterator = pd.read_csv(args.input_drugs_csv, header=None, names=['drug_id', 'smiles'], chunksize=args.drug_chunk_size, low_memory=True, engine='c')
    ...
    for chunk_num, drug_chunk_df in enumerate(chunk_iterator, 1):
        ...
        print(f"\n" + "="*20 + f" Processing Drug Chunk {chunk_num} " + "="*20)
        ...
        if molgnet_featurizer:
            # MolGNet的批处理大小现在也由--small_batch_size控制，以保持一致性
            drug_loader = PyGDataLoader([d['graph_obj'] for d in processed_drugs_graphs], batch_size=args.small_batch_size, shuffle=False)
            final_features = []
            with torch.no_grad():
        ...
        ### --- 【【【 修改点 START: 引入细胞系分块并调整批处理逻辑 】】】 --- ###
        
        print(f"Step 3: Starting prediction with drug batches of {args.small_batch_size} and cell line chunks of {args.cell_chunk_size}...")
        all_results_for_chunk = []
        
        num_drug_batches = (len(processed_drugs_graphs) + args.small_batch_size - 1) // args.small_batch_size
        for i in tqdm(range(0, len(processed_drugs_graphs), args.small_batch_size), total=num_drug_batches, desc="Drug Batches"):
            small_batch_drugs = processed_drugs_graphs[i : i + args.small_batch_size]
            
            num_cell_chunks = (len(cell_lines_to_predict) + args.cell_chunk_size - 1) // args.cell_chunk_size if args.cell_chunk_size > 0 else 1
            for k in range(num_cell_chunks):
                cell_start_idx = k * args.cell_chunk_size
                cell_end_idx = (k + 1) * args.cell_chunk_size if args.cell_chunk_size > 0 else len(cell_lines_to_predict)
                cell_line_chunk = cell_lines_to_predict[cell_start_idx:cell_end_idx]
                
                batch_pairs_data, batch_identifiers = [], []
                for drug_data in small_batch_drugs:
                    for cell_name in cell_line_chunk:
                ...
                # 【关键点】: DataLoader的批处理大小设置为当前组合中的所有配对数量。
                # 这确保了这整个 "药物小批 x 细胞区块" 的组合被当作一个大批次送入GPU。
                # 您的GPU显存需要能容纳 len(batch_pairs_data) 个样本。
                gpu_workload_size = len(batch_pairs_data)
                pairs_loader = PyGDataLoader(DrugCellDataset(batch_pairs_data), batch_size=gpu_workload_size, shuffle=False)
                
                predictions_for_batch = []
                with torch.no_grad():
                    # 这个循环只会执行一次，因为批处理大小等于数据集大小
        ...
        if all_results_for_chunk:
            ...
            try:
                ...
                print(f"\nDrug Chunk {chunk_num} results saved to {chunk_output_path}")
            except Exception as e:
                print(f"\nError saving drug chunk {chunk_num} results: {e}")
        
        t_chunk_end = time.time()
        print(f"Drug Chunk {chunk_num} finished in {t_chunk_end - t_chunk_start:.2f}s.")
...
if __name__ == '__main__':
    ...
    # --- 输入/输出参数 ---
    parser.add_argument('--input_drugs_csv', type=str, default='./CRC/drug_depmap.csv', help="包含所有待预测药物的CSV文件路径。")
    parser.add_argument('--gene_expression_file', type=str, default='./CRC/gene_all.csv', help="新细胞系的基因表达谱文件路径。")
    ...
    # --- 【【【 修改点: 移除 gpu_batch_size, 添加 cell_chunk_size 】】】 ---
    parser.add_argument('--drug_chunk_size', type=int, default=100000, help="每个药物大块的大小。")
    parser.add_argument('--small_batch_size', type=int, default=256, help="内存中一次处理的药物小批数量。这也是MolGNet的批处理大小。")
    parser.add_argument('--cell_chunk_size', type=int, default=32, help="内存中一次处理的细胞系区块大小。")
    parser.add_argument('--num_workers', type=int, default=12, help="用于药物预处理的CPU核心数 (-1 表示使用所有可用核心)。")
