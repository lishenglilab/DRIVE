...
try:
    ...
    print("ERROR: Model_GNN.py not found.");
    exit()
try:
    ...
    print(f"ERROR: Failed to import from Model.py. Details: {e}");
    exit()


class Self_loop:
    # Adds self-loops to the graph's edge index.
    def __call__(self, data):
        ...
        if hasattr(data, 'edge_attr') and data.edge_attr is not None:
            ...
            data.edge_attr = self_loop_attr
        return data


class Add_seg_id:
    # Adds segment IDs to edges and nodes.
    def __call__(self, data):
        device = data.x.device;
        data.edge_seg = torch.zeros(data.num_edges, dtype=torch.long, device=device);
        data.node_seg = torch.zeros(data.num_nodes, dtype=torch.long, device=device)
        return data
...
parser.add_argument('--gene_expression_file', type=str, default='./test/gene_sample.csv',
                    help="CSV for new cell lines.")
...
parser.add_argument('--bionic_dict_path', type=str, default='../Dataset/BIONIC_dict.pkl',
                    help="Path to BIONIC_dict.pkl.")
parser.add_argument('--canonical_gene_list_path', type=str, default='../Dataset/exp.txt',
                    help="Path to canonical gene list.")
...
try:
    ...
    print(f"Error during config/module loading: {e}");
    exit()
...
def preprocess_new_cell_data(gene_expr_path, canonical_gene_path, bionic_dict_path, gene_add_num, expected_gene_dim):
    ...
    try:
        # Use pandas to read the tab-separated file, assuming the first column is GENE_SYMBOLS
        canonical_df = pd.read_csv(os.path.abspath(canonical_gene_path), sep='\t', header=0)
        # Assume the first column contains the gene symbols
        canonical_gene_list_raw = canonical_df.iloc[:, 0].tolist()[:expected_gene_dim]
    except Exception as e:
        print(
            f"Error: Failed to read canonical gene list '{canonical_gene_path}'. Ensure it's tab-separated with a header. Error: {e}")
        # If reading fails, try the original line-reading method as a fallback
        print("Attempting to use fallback line reading method...")
        with open(os.path.abspath(canonical_gene_path), 'r', encoding='gbk', errors='ignore') as f:
            ...
            if canonical_gene_list_raw[0].lower() in ['gene_symbols', 'gene_symbol']:
                canonical_gene_list_raw.pop(0)  # Remove header
    ...
    # Create a mapping from cleaned gene names to their original indices in the canonical list
    ...
    print("\n" + "=" * 20 + " [Gene Alignment Diagnostics] " + "=" * 20)
    print(f"Number of genes in canonical list: {len(canonical_gene_set)}")
    print(f"Number of genes in your data file: {len(original_user_genes_cleaned)}")
    print(f"Number of successfully aligned (intersecting) genes: {intersection_count}")
    if intersection_count == 0:
        print("Warning: Your gene names have no intersection with the canonical list! Printing samples for comparison:")
        print("--- Canonical List Gene Name Samples (first 10): ---");
        print(canonical_gene_list_cleaned[:10])
        print("--- Your Data Gene Name Samples (first 10): ---");
        print(list(original_user_genes_cleaned)[:10])
    print("=" * 67 + "\n")
    ...
    for cell_name, row in tqdm(aligned_df.iterrows(), total=len(aligned_df), desc="Generating Cell Features"):
        ...
        for j in range(gene_add_num):
            ...
            if gene_canonical_index in bionic_dict:
                k += 1;
                feature_sum += torch.tensor(bionic_dict[gene_canonical_index], dtype=torch.float32)
...
if num_workers > 0 and platform.system() != "Linux":
    print(f"Warning: Forcing num_workers to 0 on {platform.system()} system.")
...
# Load featurizer and predictor models
molgnet_featurizer = None;
g_self_loop = Self_loop();
g_add_seg_id = Add_seg_id()
if os.path.exists(args.molgnet_model_path):
    try:
        ...
        molgnet_featurizer.to(DEVICE).eval();
        print("MolGNet featurizer loaded successfully.")
    except Exception as e:
        print(f"Error loading MolGNet featurizer: {e}"); molgnet_featurizer = None

print("Loading main predictor model...")
try:
    _, model_state, _ = joblib.load(args.model_path)
    predictor_model = Predictor(TrainConfig.embedding_dim, TrainConfig.heads, TrainConfig.fc_layer_num,
                                TrainConfig.fc_layer_dim, TrainConfig.dropout_rate)
    ...
    print(f"Error loading main predictor model: {e}"); exit()

# Main processing and prediction logic
t_overall_start = time.time()
cell_features_dict = preprocess_new_cell_data(args.gene_expression_file, args.canonical_gene_list_path,
                                              args.bionic_dict_path, getattr(DataConfig, 'gene_add_num', 500),
                                              EXPECTED_GENE_FEATURE_DIM)
...
all_drugs_initial = [{'drug_id': str(r['drug_id']).strip(), 'graph_obj': get_drug_graph(str(r['drug_id']).strip(), str(
    r['smiles']).strip() if pd.notna(r['smiles']) else None)} for _, r in
                     tqdm(input_drugs_df.iterrows(), total=len(input_drugs_df), desc="Creating Drug Graphs")]
...
if molgnet_featurizer and all_drugs_initial:
    ...
    drug_loader = PyGDataLoader([d['graph_obj'] for d in all_drugs_initial], batch_size=batch_size, shuffle=False,
                                num_workers=num_workers)
    final_features = [];
    with torch.no_grad():
        for batch in tqdm(drug_loader, desc="MolGNet Batches"):
            batch = batch.to(DEVICE);
            batch = g_self_loop(batch);
            batch = g_add_seg_id(batch)
...
for drug_data in tqdm(processed_drugs, desc="Predicting (Drug by Drug)"):
    ...
    for cell_name, cell_feats in cell_features_dict.items():
        current_drug_pairs_data.append(Data(x=drug_data['node_features'], edge_index=drug_data['graph_obj'].edge_index,
                                            edge_attr=getattr(drug_data['graph_obj'], 'edge_attr', None),
                                            GEF=cell_feats['GEF'], BNF=cell_feats['BNF']))
        current_drug_identifiers.append(
            {'input_drug_id': drug_data['drug_id'], 'resolved_drug_name': drug_data['drug_id'],
             'cell_line_name': cell_name})

    if not current_drug_pairs_data: continue
    current_drug_loader = PyGDataLoader(DrugCellDataset(current_drug_pairs_data), batch_size=batch_size, shuffle=False,
                                        num_workers=num_workers)
    ...
    with torch.no_grad():
        for batch in current_drug_loader:
            ...
            # [Critical Fix]: Ensure drug features are always 2D to fix IndexError
            ...
            predictions = predictor_model(node_features_for_model, pyg_batch_for_model, gene_ft_for_model,
                                          bionic_ft_for_model)
            predictions_for_this_drug.extend(
                torch.squeeze(predictions).cpu().tolist() if predictions.numel() > 1 else [predictions.item()])

    for i, identifier in enumerate(current_drug_identifiers):
        if i < len(predictions_for_this_drug):
            identifier['predicted_ic50'] = predictions_for_this_drug[i];
            all_results.append(identifier)
        else:
            print(f"Warning: Prediction count mismatch for drug {drug_data['drug_id']}.")
...
if all_results:
    ...
    results_df = results_df[['input_drug_id', 'resolved_drug_name', 'cell_line_name', 'predicted_ic50']]
