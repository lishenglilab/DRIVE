...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        ...
        rmse = np.sqrt(mse)
        # 计算 PCC
        pcc, _ = pearsonr(test_real, test_pre)
        scc, _ = spearmanr(test_real, test_pre)
        # 计算 R²
        ss_res = np.sum((test_real - test_pre) ** 2)
        ss_tot = np.sum((test_real - np.mean(test_real)) ** 2)
        r2 = 1 - (ss_res / ss_tot)
        # 打印指标
        print(f'Test Loss: {test_epoch_loss:.8f} MSE: {mse:.8f} RMSE: {rmse:.8f} PCC: {pcc:.8f} R2: {r2:.8f} SCC: {scc:.8f}')
        # 将所有指标和损失值保存到文件
        with open(save_path_log, 'a') as file0:
            file0.write(
                f'Test Loss: {test_epoch_loss:.8f} MSE: {mse:.8f} RMSE: {rmse:.8f} PCC: {pcc:.8f} R2: {r2:.8f} SCC: {scc:.8f}\n')
    end = time.time()
    print(end - start)
    # saving
    if epoch == 0:
